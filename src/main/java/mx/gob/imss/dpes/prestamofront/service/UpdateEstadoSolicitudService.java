 ...
 */
@Provider
public class UpdateEstadoSolicitudService extends ServiceDefinition<RequestPrestamo, RequestPrestamo> {
    ...
    public Message<RequestPrestamo> execute(Message<RequestPrestamo> request) throws BusinessException {
        ...
        log.log(Level.INFO, ">>>UpdateEstadoSolicitudService Request hacia SolicitudBack: {0}", solicitud);
        if (!(request.getPayload().getSolicitud().getCveEstadoSolicitud().getId() == TipoEstadoSolicitudEnum.PRESTAMO_RECUPERACION.toValue()
                || request.getPayload().getSolicitud().getCveEstadoSolicitud().getId() == TipoEstadoSolicitudEnum.PRESTAMO.toValue()
                || request.getPayload().getSolicitud().getCveEstadoSolicitud().getId() == (TipoEstadoSolicitudEnum.BAJA_IMPROCEDENCIA.toValue())
                || request.getPayload().getSolicitud().getCveEstadoSolicitud().getId() == (TipoEstadoSolicitudEnum.BAJA_LIQUIDACION_ANTICIPADA.toValue()))) {
            solicitud.setEstadoSolicitud(TipoEstadoSolicitudEnum.AUTORIZADO);
            solicitud.setCveEstadoSolicitud(new EstadoSolicitud(
                    TipoEstadoSolicitudEnum.AUTORIZADO.getTipo())
            );
        }
